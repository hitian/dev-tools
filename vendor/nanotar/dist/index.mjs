function parseTar(data) {
  ...
  while (offset < buffer.byteLength - 512) {
    ...
    const safeName = _sanitizeTarPath(prefix.length ? `${prefix}/${name}` : name);
...
function _sanitizeTarPath(path) {
  if (!path) {
    throw new Error("Unsafe tar path: empty path");
  }
  const normalized = path.replace(/\\/g, "/");
  if (normalized.startsWith("/") || /^[a-zA-Z]:\//.test(normalized)) {
    throw new Error("Unsafe tar path: absolute paths are not allowed");
  }
  ...
  for (const segment of segments) {
    ...
    if (segment === "..") {
      throw new Error("Unsafe tar path: path traversal is not allowed");
    }
    safeSegments.push(segment);
  }
  if (!safeSegments.length) {
    throw new Error("Unsafe tar path: empty normalized path");
  }
